...
from tests.factories import RequestScopeFactory


def test_is_htmx_request() -> None:
    request = RequestFactory(scope=RequestScopeFactory(headers=[(b"hx-request", b"yes")]))
    htmx.is_htmx_request(request)

    request = RequestFactory(scope=RequestScopeFactory(headers=[]))
    assert not htmx.is_htmx_request(request)


def test_matches_target() -> None:
    request = RequestFactory(scope=RequestScopeFactory(headers=[(b"hx-target", b"main")]))
