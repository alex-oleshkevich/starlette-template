...
from app.http.web.auth.routes import forgot_password_rate_limit
...
async def _forgot_password_rate_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.http.web.auth.routes.forgot_password_rate_limit", limits.parse("1000/second"))
...
def test_rate_limit(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("app.http.web.auth.routes.forgot_password_rate_limit", limits.parse("1/minute"))
    response = client.post("/forgot-password", data={"email": "nonexisting@user.tld"})
    assert response.status_code == 302
    response = client.post("/forgot-password", data={"email": "nonexisting@user.tld"})
