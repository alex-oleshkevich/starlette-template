...
class LoginForm(wtforms.Form):
    email = wtforms.EmailField(
        _("Email"), validators=[wtforms.validators.data_required(), wtforms.validators.length(max=255)]
    )
...
class ChangePasswordForm(wtforms.Form):
    password = wtforms.PasswordField(
        _("New password"), validators=[wtforms.validators.data_required(), wtforms.validators.length(max=255)]
    )
